...
def prepare_stack(expression: ast.Expression) -> typing.Tuple[ops.ByteCodeOp, ...]:
    if isinstance(expression, ast.ConstructionExpression):
        ...
        return sum(
            ...
            in expression.arguments.values()),
            start=tuple(),
        ) + (
        ...
        return (ops.PushFromLiteralOp(value=TaggedValue(tag=float, value=expression.value)),)
        ...
        return prepare_stack(expression.lhs) + prepare_stack(expression.rhs) + operator_ops(expression.operator)
...
def compile_statement_ast(
    ...
    if isinstance(statement, ast.AssignmentStatement):
        return prepare_stack(statement.expression) + (ops.PopToNameOp(name=statement.variable_name),)
...
def compile_function_ast(
    ...
    return Subroutine(
        ops=sum(
            (compile_statement_ast(statement)
...
def compile_ast(program: ast.Program) -> Program:
    ...
    subroutines = {
        function.name:
        compile_function_ast(function, structure_lookup)
        for function
